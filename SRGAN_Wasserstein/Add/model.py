...
def SRGAN_d(input_images, model,is_train=True, reuse=False):
    w_init = tf.random_normal_initializer(stddev=0.02)
    b_init = None # tf.constant_initializer(value=0.0)
...
def Res_SRGAN_g(t_image, is_train=False, reuse=False):
    """ Generator in Photo-Realistic Single Image Super-Resolution Using a Generative Adversarial Network
    feature maps (n) and stride (s) feature maps (n) and stride (s)
    """
    w_init = tf.random_normal_initializer(stddev=0.02)
    b_init = None # tf.constant_initializer(value=0.0)
    g_init = tf.random_normal_initializer(1., 0.02)
    with tf.variable_scope("Res_SRGAN_g", reuse=reuse) as vs:
        tl.layers.set_name_reuse(reuse)
        n = InputLayer(t_image, name='in')
        n = Conv2d(n, 64, (9, 9), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, name='n64s1/c')
        temp = n
...
def Res_SRGAN_d(input_images, is_train=True, reuse=False):
    w_init = tf.random_normal_initializer(stddev=0.02)
    b_init = None # tf.constant_initializer(value=0.0)
    gamma_init=tf.random_normal_initializer(1., 0.02)
    df_dim = 64
    lrelu = lambda x: tl.act.lrelu(x, 0.2)
    with tf.variable_scope("resSRGAN_d", reuse=reuse):
        tl.layers.set_name_reuse(reuse)
        '''net_in = InputLayer(input_images, name='input/images')
        net_h0 = Conv2d(net_in, df_dim, (4, 4), (2, 2), act=lrelu,
                padding='SAME', W_init=w_init, name='h0/c')
        ...
        n = ElementwiseLayer([n, temp], tf.add, 'add3')
        # B residual blacks end
        n.outputs = tl.act.lrelu(n.outputs, 0.2)

        net_ho = FlattenLayer(n, name='ho/flatten')
        net_ho = DenseLayer(net_ho, n_units=1, act=tf.identity,
                W_init = w_init, name='ho/dense')
        logits = net_ho.outputs
        sigmoid_out = tf.nn.sigmoid(net_ho.outputs)
        # Wasserstein GAN doesn't need the sigmoid output
        # net_ho.outputs = tf.nn.sigmoid(net_ho.outputs)
        #logits = n.outputs
        #n.outputs = tf.nn.sigmoid(n.outputs)

    return net_ho, logits ,sigmoid_out
